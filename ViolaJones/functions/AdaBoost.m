function [Weak_theta, Weak_pol, Alpha, WeakClassifiers] = AdaBoost(All_features, Labels, numFaceImages,numBgImages, numWeakClass)
...
for t=1:numWeakClass
    ...
    [minError, WeakClassifiers(t)] = min(error);
    
    %Store the information needed for this weak classifier    
    Weak_theta(t) = theta(WeakClassifiers(t));
    Weak_pol(t) = polarity(WeakClassifiers(t));
    weak_error(t) = minError;
    Beta(t) = minError/(1 - minError);
    
    %Determine how many images were classified correctly
    if Weak_pol(t) == 1
        h = (All_features(WeakClassifiers(t),:) > Weak_theta(t));
    else
        h = (All_features(WeakClassifiers(t),:) < Weak_theta(t));
    end
    
    correct(t) = sum(h==Labels);
    e = (h~=Labels);

    
    %Update Weights for next weak classifier
    weights = weights.*Beta(t).^(1-e);
